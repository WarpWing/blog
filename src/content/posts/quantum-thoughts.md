...
He also emphasized that even if someone proves P equals NP, it would only disrupt current encryption if the polynomial-time algorithms discovered are practical—that is, they run efficiently enough to be usable in real-world applications (like time complexity proportional to n^3). If the polynomial algorithm has an impractically high degree (e.g., time complexity proportional to n^1000), it wouldn’t pose an immediate threat to cryptographic systems.
